...
#define HAVALVE_CALLBACK(name) void (*name)(bool state, HAValve* sender)

/**
 * HAValve allows to display on/off switch in the HA panel and receive commands on your device.
 *
 * @note
...
public:
     ...
     *
     * @param callback
     * @note In non-optimistic mode, the state must be reported back to HA using the HAValve::setState method.
     */
